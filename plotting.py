...
def retrieve_external_data(
    ...
    all_exfortables_files = sorted(all_exfortables_files, key=extract_year_from_filename)

    if not all_exfortables_files:
        logging.info(f"No experimental data files found in the directory: {exfortables_directory}")
        return
    ...
    for file in all_exfortables_files:
        cleaned_all_exfortables_file = os.path.join(
            cleaned_all_exfortables_directory, f"cleaned_{os.path.basename(file)}",
        )
        clean_data_file(file, cleaned_all_exfortables_file)
        cleaned_all_exfortables_files.append(cleaned_all_exfortables_file)

    logging.info(f"Found {len(cleaned_all_exfortables_files)} external data files for plotting.")
...
def extract_label_from_filename(filename, cleaned_selected_exfortables_files):
    ...
    if len(parts) >= 5:
        ...
        cleaned_base_names = [os.path.basename(file) for file in cleaned_selected_exfortables_files]
...
def generate_chi_squared_gnuplot_script(
        chi2_values, 
        plot_file,
        element,
        mass,
...
def generate_ratio_gnuplot_script(chi2_values_list, output_file, numerator_idx, denominator_idx):
    gnuplot_script = f"""
...
"""
    ...
    for array in chi2_values_list:
        if array[denominator_idx] != 0:
            gnuplot_script += f"{array[0]} {array[numerator_idx]/array[denominator_idx]}\n" if array[denominator_idx] !=0 else ""
    gnuplot_script += "e\n"
