...
import colorsys
import numpy as np
from create_projectile_files import create_projectile_file1, create_projectile_file2, create_projectile_file3, create_projectile_file4, create_projectile_file5
...
def calculate_combined_chi_squared(combined_directory, cleaned_external_files, simulation_data):

    chi_squared = 0.0
    dataset_chi_squared_list = []
    valid_datasets = 0.0
    output_file_path = os.path.join(combined_directory, "chi_squared_values.txt")
...
def main():
    ...
    product_six_digit_code3 = input("Enter the Z&A of product3 in six digits: ")

    # Define the combined output directory
    home_directory = os.path.expanduser("~")  
    combined_directory = os.path.join(os.path.expanduser("~"), "Documents", "talys", f"{projectile}-{element}{mass}_chisquared_triple")
    os.makedirs(combined_directory, exist_ok=True)
    
    cleaned_external_files1 = []
    cleaned_external_files2 = []
    cleaned_external_files3 = []
    ...
    external_directory3 = os.path.join(home_directory, "Documents", "exfortables", f"{projectile}", f"{element_capitalized}"f"{formatted_mass}", "residual", product_six_digit_code3)
    ...
    retrieve_external_data(external_directory3, combined_directory, cleaned_external_files3)
    cleaned_output_files1 = []
    cleaned_output_files2 = []
    cleaned_output_files3 = []
    chi2_values1 = []
    chi2_values2 = []
    chi2_values3 = []
    chi2_value_averages = []
    ...
    for i in range(1, 6):  # Loop through 1 to 5
        function_name = f"create_projectile_file{i}"  # Create the function name dynamically as a string
        ...
        if not data_file2:
            ...
            return
        data_file3 = search_for_specific_output_file(new_directory, product_six_digit_code3)
        if not data_file2:
            print(f"No 'rp*' files found with the six-digits '{product_six_digit_code3}' in the directory.")
            return
        ...
        cleaned_output_file3 = os.path.join(new_directory, f"cleaned_{os.path.basename(data_file3)}")
        clean_data_file(data_file3, cleaned_output_file3)
        cleaned_output_files3.append(cleaned_output_file3)

        simulation_data1 = load_simulation_data(cleaned_output_file1)  # Load the cleaned .tot file
        combined_chi_squared1 = calculate_combined_chi_squared(combined_directory, cleaned_external_files1, simulation_data1)
        ...
        combined_chi_squared2 = calculate_combined_chi_squared(combined_directory, cleaned_external_files2, simulation_data2)
        chi2_values2.append(combined_chi_squared2)

        simulation_data3 = load_simulation_data(cleaned_output_file3)  # Load the cleaned .tot file
        combined_chi_squared3 = calculate_combined_chi_squared(combined_directory, cleaned_external_files3, simulation_data3)
        chi2_values3.append(combined_chi_squared3)

        chi2_value_average = (combined_chi_squared1 + combined_chi_squared2 + combined_chi_squared3) / 3.0
    ...
    plot_file = os.path.join(combined_directory, f"combined_cross_section_plot_{product_six_digit_code3}.png")
    gnuplot_script3 = generate_combined_gnuplot_script(cleaned_output_files3, cleaned_external_files3, plot_file)
    gnuplot_script_file3 = os.path.join(combined_directory, f"combined_cross_section_plot_{product_six_digit_code3}.gp")
    ...
    gnuplot_script2 = generate_chi_squared_gnuplot_script(chi2_values1, chi_squared_plot_file)
    gnuplot_script_file2 = os.path.join(combined_directory, "chi_squared_vs_m2constant1.gp")
    run_gnuplot(gnuplot_script2, gnuplot_script_file2)   

    chi_squared_plot_file = os.path.join(combined_directory, f"chi_squared_vs_m2constant_{product_six_digit_code2}.png")
    gnuplot_script2 = generate_chi_squared_gnuplot_script(chi2_values2, chi_squared_plot_file)
    gnuplot_script_file2 = os.path.join(combined_directory, "chi_squared_vs_m2constant2.gp")
    run_gnuplot(gnuplot_script2, gnuplot_script_file2) 

    chi_squared_plot_file = os.path.join(combined_directory, f"chi_squared_vs_m2constant_{product_six_digit_code3}.png")
    gnuplot_script3 = generate_chi_squared_gnuplot_script(chi2_values3, chi_squared_plot_file)
    gnuplot_script_file3 = os.path.join(combined_directory, "chi_squared_vs_m2constant3.gp")
    run_gnuplot(gnuplot_script3, gnuplot_script_file3) 

    chi_squared_combined_plot_file = os.path.join(combined_directory, f"chi_squared_average_vs_m2constant.png")
