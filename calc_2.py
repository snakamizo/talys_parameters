...
def process(input, score_dict):
    ...
    for j, code in enumerate(product_six_digit_codes):
        plot_file = os.path.join(
            gnuplot_output_directory, f"combined_cross_section_plot_{code}.png"
        )
        ...
        gnuplot_script_file = os.path.join(
            gnuplot_output_directory, f"combined_cross_section_plot_{code}.gp"
        )
...
def main():
    ...
    with ProcessPoolExecutor(max_workers=N) as executor:
        futures = [executor.submit(process, input, score_dict) for input in medical_isotope_reactions]
