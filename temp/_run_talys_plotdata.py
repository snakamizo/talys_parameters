...
def rgb_to_hex(r, g, b):
    return f'#{r:02X}{g:02X}{b:02X}'  # Converts RGB values to hex format

def generate_combined_gnuplot_script(data_files, cleaned_external_files, talys_output_file, plot_file):
...
"""
    ...
    for index, cleaned_ext_file in enumerate(cleaned_external_files):
        ext_label = extract_label_from_filename(cleaned_ext_file)  # Use formatted label
        ...
        plot_lines.append(f"'{cleaned_ext_file}' using 1:3:4 with errorbars title '{ext_label}' pt {point_type} lc rgb '{color}'")
...
def main():
    ...
    new_directory = os.path.join(home_directory, "Documents", "talys", f"{projectile}-{element}{mass}_plotdata")
    ...
    # Step 4: Add TALYS output to the plot if available
    talys_output_file = os.path.join(new_directory, f"cleaned_rp{product_six_digit_code}.tot")  # Adjust based on actual output location
    ...
    gnuplot_script = generate_combined_gnuplot_script(cleaned_files, cleaned_external_files, talys_output_file, plot_file)
    ...
    print("Data files for plot:", data_files)
    print("External files for plot:", external_files)
    print("TALYS output file:", talys_output_file)
