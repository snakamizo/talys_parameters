...
def interpolate_simulation(energy_exp, simulation_data):
    ...
    logging.debug(f"Interpolating for energy: {energy_exp} with simulation data: {simulation_data[:5]}...")
...
def calculate_combined_chi_squared(
    ...
    output_file_path = os.path.join(
        output_directory, f"chi_squared_values_{projectile}-{target[1]}{target[0]}{code}_{i}.txt"
    )

    logging.debug(f"Starting chi-squared calculation for {len(cleaned_external_files)} files.")
    ...
    with open(output_file_path, "w") as output_file:
        ...
        for cleaned_external_file in cleaned_external_files:
            logging.debug(f"Loading experimental data from file: {cleaned_external_file}")
            experimental_data = load_experimental_data(
                cleaned_external_file
            )
            logging.debug(f"Experimental data points loaded: {len(experimental_data)} from {cleaned_external_file}")
            valid_points = 0.0
            ...
            for exp_point in experimental_data:
                ...
                if delta_cross_exp < ERROR_THRESHOLD * cross_section_exp or cross_section_exp < XS_THRESHOLD_PERCENTAGE * 0.01 * cross_section_sim_max:
                    logging.info(
            ...
            if valid_points > 0:
                ...
                logging.info(f"Valid points for {cleaned_external_file}: {valid_points}")
                logging.info(
    ...
    logging.debug(f"Final combined chi-squared: {chi_squared}, from {valid_datasets} datasets.")
    return chi_squared
