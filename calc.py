...
def get_IAEA_medical_isotope_nuclides() -> list:
    ...
    for line in f.readlines():
        ...
        if (target[1] == '000' or projectile == 'g' or projectile == 'n' or
            projectile == 'd' or projectile == 'p' or projectile == 'h' or
            (target == ['I', '127', ''] and residual == ['Xe', '122', '']) or 
            residual == ['Ba', '128', ''] or 
            (projectile == 'p' and target == ['Th', '232', ''] and residual == ['Ra', '225', '']) or
            (projectile == 'p' and target == ['Tl', '203', ''] and residual == ['Pb', '202', 'm']) or
            (projectile == 'd' and target == ['Yb', '176', ''] and residual == ['Lu', '177', 'g']) or
            (target == ['Th', '232', ''] and residual == ['Ac', '225', '']) or
            (target == ['Th', '232', ''] and residual == ['Th', '227', ''])):
...
def process(input, score_dict, gnuplot_output_directory, chi2_value_total_averages, chi2_values_list):
    ...
    logging.info(f"chi2_values_array = {chi2_values_array}, type: {chi2_values_array.dtype}")
    for idx, value in enumerate(chi2_values_array):
        chi2_value_total_averages[idx] += value
    
    
    chi2_value_array_with_mass = np.concatenate([np.array([residual_mass], dtype=np.float64), chi2_values_array])
    logging.info(f"chi2_value_array_with_mass: {chi2_value_array_with_mass}")
    chi2_values_list.append(chi2_value_array_with_mass)

    gnuplot_each_output_directory = os.path.join(gnuplot_output_directory, f"gnuplot_output_{projectile}-{mass}{element}_{residual[1]}{residual[2]}{residual[0]}")
    os.makedirs(gnuplot_each_output_directory, exist_ok=True)

    
    plot_file = os.path.join(
        gnuplot_each_output_directory, f"combined_cross_section_plot_{product_six_digit_code}.png"
    )
    ...
    gnuplot_script_file = os.path.join(
        gnuplot_each_output_directory, f"combined_cross_section_plot_{product_six_digit_code}.gp"
    )
    ...
    chi_squared_plot_file = os.path.join(
        gnuplot_each_output_directory, f"chi_squared_vs_input_{product_six_digit_code}.png"
    )
    ...
    add_to_latex_document1(gnuplot_output_directory, gnuplot_each_output_directory, projectile, mass, element, residual)
...
# ]
    ...
    gnuplot_output_directory = os.path.join(CALC_PATH, "gnuplot_output_excluding0_Th_out")
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=N) as executor:
        ...
        with tqdm(total=len(medical_isotope_reactions), desc="Processing reactions") as progress_bar:
            for input in medical_isotope_reactions:
                target_identifier = f"{input['projectile']}-{input['element']}{input['mass']}"
    ...
    for input in skipped_reactions:
        try:
            process(input, score_dict, gnuplot_output_directory, chi2_value_total_averages, chi2_values_list)
        except Exception as e:
            logging.error(f"Error retrying {input}: {e}")

    chi2_values_list = [
        arr for arr in chi2_values_list 
        if not np.all(arr[1:6] == 0)
    ]
    ...
    for j in range(1,6):
        ...
        gnuplot_script5 = generate_mass_chi_squared_gnuplot_script(chi2_values_list, chi_squared_value_mass_plot_file, j)
    ...
    for ratio_idx, column_idx in enumerate([1, 3, 4, 5], start=1):
        ratio_plot_file = os.path.join(gnuplot_output_directory, f"ratio_vs_mass_col{column_idx}.png")
        gnuplot_script_ratio = generate_ratio_gnuplot_script(chi2_values_list, ratio_plot_file, numerator_idx=column_idx, denominator_idx=2)
        gnuplot_script_file_ratio = os.path.join(gnuplot_output_directory, f"ratio_vs_mass_col{column_idx}.gp")
