...
"""
    ...
    for i, data_file in enumerate(data_files):
        label = f"M2constant {round(1 - i * 0.1, 1)}" #Adjusting the label based on M2constant values
        if i == 0:
...
def main():
    ...
    constants = [round(x, 1) for x in list(frange(1, 0.1, -0.1))]

    # Define the combined output directory
    combined_directory = os.path.join(os.path.expanduser("~"), "Documents", "talys", f"{projectile}-{element}{mass}-({product_six_digit_code})_M2")
